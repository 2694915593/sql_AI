...
class ParamExtractor(LogMixin):
    ...
    def __init__(self, sql_text: str, logger=None, metadata_list=None, config_manager=None, db_alias: Optional[str] = None):
        """
        初始化参数提取器
        ...
        self.logger.debug(f"参数提取器初始化，原始SQL长度: {len(self.original_sql)}, 处理后SQL长度: {len(self.sql_text)}, 表元数据数量: {len(self.metadata_list)}, db_alias: {db_alias}")
    ...
    def generate_replaced_sql(self) -> tuple:
        ...
        try:
            ...
            # 使用已经获取到的数据库实例信息（优先使用初始化时传入的db_alias）
            db_alias = self.db_alias
            if not db_alias and self.config_manager:
                # 如果没有传入db_alias，则使用原来的逻辑获取默认数据库别名
                try:
                    target_dbs = self.config_manager.get_all_target_db_aliases()
                    if target_dbs:
                        db_alias = target_dbs[0]
                        self.logger.debug(f"没有传入db_alias，使用默认数据库别名: {db_alias}")
                except Exception as e:
                    self.logger.warning(f"获取数据库别名失败: {str(e)}")
            elif db_alias:
                self.logger.debug(f"使用传入的数据库别名: {db_alias}")
            
            if not db_alias:
                self.logger.warning("没有可用的数据库别名，可能无法获取数据值")
            ...
            self.logger.info(f"生成替换参数后的SQL，成功替换了 {len(params)} 个参数，使用数据库别名: {db_alias}")
