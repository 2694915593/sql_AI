...
# 直接导入utils模块
utils_dir = os.path.join(sql_ai_analyzer_dir, 'utils')
...
try:
    ...
    print(f"检查文件是否存在:")
    print(f"  - config_manager.py: {os.path.exists(os.path.join(sql_ai_analyzer_dir, 'config', 'config_manager.py'))}")
    print(f"  - data_value_fetcher.py: {os.path.exists(os.path.join(sql_ai_analyzer_dir, 'data_collector', 'data_value_fetcher.py'))}")
    print(f"  - dynamic_sql_parser.py: {os.path.exists(os.path.join(sql_ai_analyzer_dir, 'data_collector', 'dynamic_sql_parser.py'))}")
    sys.exit(1)
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('test_optimizations_fixed.log')
        ]
    )
