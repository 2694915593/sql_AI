...
from sql_ai_analyzer.utils.logger import LogMixin
from sql_ai_analyzer.utils.sql_specifications import SQLType, integrate_specifications_into_prompt
import ast
...
class ModelClient(LogMixin):
    ...
    def _build_request_payload(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if tables:
            ...
            for i, table in enumerate(tables, 1):
                ...
                prompt_parts.append(f"  - 是否大表：{'是' if table.get('is_large_table', False) else '否'}")
                prompt_parts.append(f"  - 列数：{len(table.get('columns', []))}")
                prompt_parts.append(f"  - 索引数：{len(table.get('indexes', []))}")
                prompt_parts.append(f"  - 表存在性：{'是' if table.get('table_exists', True) else '否'}")
        ...
        # 只保留前5个示例
        for example in dynamic_sql_examples[:5]:
        ...
        if execution_plan:
            ...
            prompt_parts.append(f"执行计划类型：{execution_plan_info.get('db_type', '未知')}")
            
            # 优先输出执行计划摘要，降低提示词长度并突出风险
            plan_summary = execution_plan_info.get('plan_summary', {})
            if isinstance(plan_summary, dict) and plan_summary:
                prompt_parts.append("执行计划摘要：")
                prompt_parts.append(
                    f"- 访问类型: {', '.join(plan_summary.get('access_types', [])) or '未知'}"
                )
                prompt_parts.append(
                    f"- 预估扫描行数: {plan_summary.get('estimated_rows', 0)}"
                )
                prompt_parts.append(
                    f"- 是否使用索引: {'是' if plan_summary.get('key_used', False) else '否'}"
                )
                prompt_parts.append(
                    f"- 是否疑似全表扫描: {'是' if plan_summary.get('has_full_scan', False) else '否'}"
                )
            ...
            prompt_parts.append("执行计划明细：")
            formatted_plan = execution_plan_info.get('formatted_plan', '')
            if formatted_plan:
                # 防止明细过长导致prompt膨胀
                if len(formatted_plan) > 4000:
                    prompt_parts.append(formatted_plan[:4000] + "\n...（执行计划明细已截断）")
                else:
                    prompt_parts.append(formatted_plan)
            else:
                # 如果没有格式化计划，使用原始执行计划数据
                prompt_parts.append(json.dumps(execution_plan_info.get('execution_plan', ''), ensure_ascii=False, indent=2))
        else:
            # 简要说明
            prompt_parts.append("未能获取执行计划详细信息")
        
        # 6. SQL规范性评审重点 - 仅关注9个关键规范性角度
        prompt_parts.append("\n请只关注以下9个关键规范性角度的分析，忽略传统的高中低风险分类：")
        prompt_parts.append("1. 修改列时加属性：检查ALTER TABLE语句修改列时是否保留了原列的属性（如NOT NULL、DEFAULT、COMMENT等）")
        prompt_parts.append("2. in操作索引失效：检查IN操作是否导致索引失效（特别是IN列表值过多时）")
        prompt_parts.append("3. 字符集问题：检查字符集是否一致（表、字段、连接字符集），避免乱码和性能问题")
        prompt_parts.append("4. 注释--问题：检查SQL注释是否正确使用--格式（--后面必须有空格），避免注释错误解析")
        prompt_parts.append("5. comment问题：检查表和字段是否有合适的COMMENT注释，注释内容是否清晰明确")
        prompt_parts.append("6. 表参数问题：检查表参数设置是否合理（如ENGINE、CHARSET、COLLATE、ROW_FORMAT等）")
        prompt_parts.append("7. akm接入：检查AKM（访问密钥管理）相关配置和接入问题，确保敏感信息安全")
        prompt_parts.append("8. analyze问题：检查ANALYZE统计信息是否准确，是否需要手动更新统计信息")
        prompt_parts.append("9. dml与ddl之间休眠3秒：检查DDL操作后是否等待足够时间（建议3秒）再执行DML，避免锁冲突")
        prompt_parts.append("\n要求：针对每个角度，必须提供具体的、可操作的检查结果和改进建议。")
        prompt_parts.append("例如：")
        prompt_parts.append("  - 对于'修改列时加属性'：检查ALTER TABLE语句是否保留了原列的NOT NULL、DEFAULT、COMMENT等属性，如果没有，提供具体的修复SQL")
        prompt_parts.append("  - 对于'in操作索引失效'：检查IN操作是否导致索引失效，如果是，建议优化方案（如使用EXISTS、JOIN或拆分查询）")
        prompt_parts.append("  - 对于'字符集问题'：检查表、字段、连接的字符集是否一致，如果不一致，提供具体的ALTER语句修复")
        prompt_parts.append("  - 对于'注释--问题'：检查--后面是否有空格，如果没有，提供正确的注释格式")
        prompt_parts.append("  - 对于'comment问题'：检查表和字段是否有合适的COMMENT，如果没有，提供添加COMMENT的SQL")
        prompt_parts.append("  - 对于'表参数问题'：检查表参数设置是否合理，如ENGINE、CHARSET等，提供优化建议")
        prompt_parts.append("  - 对于'akm接入'：检查是否有敏感信息直接暴露在SQL中，建议使用参数化查询")
        prompt_parts.append("  - 对于'analyze问题'：检查统计信息是否准确，建议是否需要执行ANALYZE TABLE")
        prompt_parts.append("  - 对于'dml与ddl之间休眠3秒'：检查DDL和DML操作之间是否有足够间隔，建议添加适当的休眠或事务控制")
        
        # 7. 集成SQL规范要求
        prompt_parts = integrate_specifications_into_prompt(prompt_parts, sql_type)
        
        # 8. 分析要求 - 简化为只关注规范性评审
        prompt_parts.append("\n请基于以上规范性评审重点分析SQL语句：")
        prompt_parts.append("1. SQL类型识别：判断是查询/更新/插入/删除/建表/其他")
        prompt_parts.append("2. 规范性评审：针对上述15个关键角度逐一进行评审")
        prompt_parts.append("3. 建议改进：提供具体的改进建议（仅针对规范性问题的建议）")
        prompt_parts.append("4. 综合评分：给出综合评分（0-10分），基于规范性评审结果")
        prompt_parts.append("5. 规范符合度：计算SQL规范的符合度（0-100%）")
        prompt_parts.append("注意：不需要进行传统的高、中、低风险评估，仅关注上述规范性角度")
        
        # 9. 输出格式要求 - 增强格式包含规范性评审和规范符合性
  ...
  "规范性评审": {
    "修改列时加属性": {
      ...
      "details": "具体的问题描述或通过情况说明",
      "suggestion": "具体的改进建议或继续保持良好的习惯"
    },
    "in操作索引失效": {
      "status": "通过/未通过/未涉及",
  ...
  "修改建议": {
    ...
    "性能优化SQL": "针对性能问题优化后的具体SQL语句示例"
        ...
        prompt_parts.append("\n重要提示：")
        prompt_parts.append("1. 综合评分范围是0-10分，分数越高表示SQL质量越好")
        prompt_parts.append("2. 规范符合度是百分比，表示SQL符合相关规范的程度")
        prompt_parts.append("3. 规范性评审必须包含所有15个关键角度，每个角度必须有status、description、details、suggestion字段")
        prompt_parts.append("4. status字段只能是'通过'、'未通过'或'未涉及'三种状态")
        prompt_parts.append("5. 对于未涉及的角度，details和suggestion可以简单说明不适用原因")
        prompt_parts.append("6. 修改建议字段必须提供具体的、可执行的SQL语句")
        prompt_parts.append("7. 所有修改建议SQL必须是可执行的、语法正确的SQL语句")
        prompt_parts.append("8. 建议字段提供3-5条具体的改进建议")
        prompt_parts.append("9. 分析摘要要简洁明了，包含SQL的主要问题和优化建议")
        prompt_parts.append("10. 特别关注规范性评审，确保每个关键角度都有明确的评审结果")
        ...
        self.logger.info(
            "构建SQL规范集成版prompt完成：长度=%s, SQL类型=%s, 预览前500字符=\n%s",
            len(prompt),
            sql_type.value,
            prompt_preview_head
        )
        ...
        self.logger.debug(f"构建SQL规范集成请求负载: SQL长度={len(sql_statement)}, SQL类型={sql_type.value}, 表数量={len(tables)}, prompt长度={len(prompt)}")
        return payload
    ...
    def _fix_insert_structure_if_needed(self, sql_text: str) -> str:
        ...
        if open_paren == 0 and close_paren == 0:
            ...
            # 尝试找到表名
            import re
            table_match = re.search(r'insert\s+into\s+([^\s(]+)', sql_lower, re.IGNORECASE)
            if table_match:
                ...
                if param_start > 0:
                    ...
                    for i, line in enumerate(cleaned_lines):
                        if i < len(cleaned_lines) - 1:
                            columns_with_commas.append(f"{line},")
                        else:
                            columns_with_commas.append(line)
                    
                    columns_clean = '\n'.join(columns_with_commas)
                    
                    # 清理值部分 - 移除末尾的逗号，保持换行符但添加逗号
                    ...
                    for i, line in enumerate(cleaned_lines):
                        if i < len(cleaned_lines) - 1:
                            values_with_commas.append(f"{line},")
                        else:
                            values_with_commas.append(line)
                    
                    values_clean = '\n'.join(values_with_commas)
                    
                    # 检查是否需要添加VALUES关键字
                    has_values = 'values' in sql_lower
                    if not has_values:
                        self.logger.debug("缺少VALUES关键字，将添加")
                    ...
                    self.logger.debug("未找到参数分界点，无法修复")
