...
class ModelClient(LogMixin):
    ...
    def _build_request_payload(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 6. SQL评审规则（简化版，只包含关键评审点）
        prompt_parts.append("\n请根据以下SQL评审重点进行分析：")
        
        # 关键评审点
        prompt_parts.append("1. 主键检查：表是否有主键")
        prompt_parts.append("2. 索引检查：表是否有合适的索引")
        prompt_parts.append("3. 数据量评估：表数据量是否合理")
        prompt_parts.append("4. 注释检查：表和字段是否有注释")
        prompt_parts.append("5. 字段类型检查：字段类型是否合理")
        prompt_parts.append("6. 数据核对：是否有数据核对流程")
        prompt_parts.append("7. 备份策略：是否有备份策略")
        prompt_parts.append("8. SQL注入风险：是否存在SQL注入风险")
        prompt_parts.append("9. 执行性能：SQL执行性能是否良好")
        ...
        # 添加相关规范（限制数量避免提示词过长）
        for rule in relevant_rules[:20]:  # 最多20条规范
            level_str = rule["level"].value if isinstance(rule["level"], RuleLevel) else rule["level"]
            prompt_parts.append(f"{rule['id']} 【{level_str}】: {rule['content']}")
        
        prompt_parts.append("")
        prompt_parts.append("基于以上SQL规范的分析要求：")
        prompt_parts.append("1. 检查SQL语句是否符合所有相关规范")
        prompt_parts.append("2. 如果违反规范，提供具体的修改建议")
        prompt_parts.append("3. 在修改建议SQL中确保符合所有相关规范")
        prompt_parts.append("4. 在风险评估中标注违反规范的问题")
        prompt_parts.append("5. 在综合评分中考虑规范符合性")
        prompt_parts.append("")
        
        # 8. 分析要求（更新版）
        prompt_parts.append("\n请基于以上评审重点和SQL规范分析SQL语句：")
        prompt_parts.append("1. SQL类型识别：判断是建表、表结构变更、索引操作还是数据操作")
        prompt_parts.append("2. 规范符合性检查：根据SQL类型检查对应的SQL规范")
        prompt_parts.append("3. 风险评估：识别潜在的风险和问题，包括违反规范的风险")
        prompt_parts.append("4. 建议改进：提供具体的改进建议，确保符合规范")
        prompt_parts.append("5. 综合评分：给出综合评分（0-10分），考虑规范符合性")
        
        # 9. 输出格式要求 - 新的简化格式
        prompt_parts.append("\n请严格按照以下简化的JSON格式回复，不要包含任何其他内容：")
        prompt_parts.append('''{
  ...
  "修改建议": {
    ...
    "性能优化SQL": "针对性能问题优化后的具体SQL语句示例"
        ...
        prompt_parts.append("2. 风险评估中的问题需要具体明确，不要泛泛而谈")
        prompt_parts.append("3. 修改建议字段必须提供具体的、可执行的SQL语句：")
        prompt_parts.append("   - 高风险问题SQL：针对高风险问题，提供修复后的完整SQL语句")
        prompt_parts.append("   - 中风险问题SQL：针对中风险问题，提供优化后的SQL语句")
        prompt_parts.append("   - 低风险问题SQL：针对低风险问题，提供改进后的SQL语句")
        prompt_parts.append("   - 性能优化SQL：针对性能问题，提供优化后的SQL语句")
        prompt_parts.append("4. 所有修改建议SQL必须是可执行的、语法正确的SQL语句")
        prompt_parts.append("5. 建议字段提供3-5条具体的改进建议")
        prompt_parts.append("6. 分析摘要要简洁明了，包含SQL的主要问题和优化建议")
        ...
        self.logger.debug(f"构建简化请求负载: SQL长度={len(sql_statement)}, 表数量={len(tables)}, prompt长度={len(prompt)}")
        return payload
    ...
    def _fix_sql_structure_if_needed(self, sql_text: str) -> str:
        ...
        fixed = self._fix_comma_before_where(fixed)

        return fixed

    def _fix_comma_before_where(self, sql_text: str) -> str:
        """
        修复UPDATE语句WHERE前的多余逗号问题
        ...
        # 查找WHERE前的逗号
        comma_before_where = -1
        # 从WHERE向前搜索第一个非空白字符
        i = where_pos - 1
        while i >= 0 and sql_text[i].isspace():
            i -= 1
        
        # 如果找到逗号
        if i >= 0 and sql_text[i] == ',':
            comma_before_where = i
            # 继续向前检查是否有多个连续的逗号
            while i >= 0 and (sql_text[i] == ',' or sql_text[i].isspace()):
                i -= 1
            
            # 重新定位逗号位置（最后一个逗号）
            comma_before_where = i + 1
            
            # 移除多余逗号
            if comma_before_where >= 0:
                # 保留逗号前的所有内容，然后连接到WHERE部分
                before_comma = sql_text[:comma_before_where].rstrip()
                after_comma = sql_text[comma_before_where + 1:]  # 跳过逗号
                
                # 移除逗号后的空白
                after_comma_stripped = after_comma.lstrip()
                
                # 重新组合SQL
                fixed_sql = before_comma + ' ' + after_comma_stripped
                
                self.logger.debug(f"修复UPDATE语句WHERE前多余逗号: 原始长度={len(sql_text)}, 修复后长度={len(fixed_sql)}")
                return fixed_sql
        
        # 另一种情况：WHERE前可能有多个逗号或逗号+空白
        # 使用正则表达式匹配 SET ... , WHERE 模式
        import re
        
        # 匹配 SET 部分到 WHERE 之间的模式，捕获SET部分和WHERE部分
        # 这个模式匹配 SET column = value, ... , WHERE
        # 会捕获 SET 后的所有内容直到 WHERE（包括WHERE前的逗号）
        pattern = r'(SET\s+.*?)\s*,\s*(WHERE\s+.*)'
        ...
        if match:
            ...
            # 重建SQL：移除逗号，直接将SET部分连接到WHERE部分
            fixed_sql = sql_text[:match.start()] + set_part + ' ' + where_part
            
            self.logger.debug(f"使用正则修复UPDATE语句WHERE前多余逗号: 原始长度={len(sql_text)}, 修复后长度={len(fixed_sql)}")
            return fixed_sql
        
        # 第三种情况：检查是否有 SET ... , , WHERE 这样的多重逗号
        # 使用更简单的正则，匹配连续逗号
        pattern2 = r'(SET\s+.*?)\s*,\s*,\s*(WHERE\s+.*)'
        match2 = re.search(pattern2, sql_text, re.IGNORECASE | re.DOTALL)
        
        if match2:
            set_part = match2.group(1)
            where_part = match2.group(2)
            
            # 移除一个逗号
            fixed_sql = sql_text[:match2.start()] + set_part + ' ' + where_part
            
            self.logger.debug(f"修复UPDATE语句WHERE前多重逗号: 原始长度={len(sql_text)}, 修复后长度={len(fixed_sql)}")
            return fixed_sql
