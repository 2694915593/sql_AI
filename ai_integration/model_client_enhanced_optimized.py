...
from sql_ai_analyzer.utils.specifications_prompt_enhanced import get_specifications_prompt
import ast
...
class ModelClient(LogMixin):
    ...
    def __init__(self, config_manager, logger=None):
        ...
        self.logger.info("大模型API客户端初始化完成（优化版）")
    ...
    def _build_request_payload(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 获取增强版规范提示词
        specifications_prompt = get_specifications_prompt(sql_type)
        prompt_parts.append(specifications_prompt)
        
        # 7. 分析要求 - 简化
        prompt_parts.append("\n分析要求：")
        prompt_parts.append("1. SQL类型识别：判断是查询/更新/插入/删除/建表/其他")
        prompt_parts.append("2. 规范符合性检查：检查SQL语句是否符合上述53条规范")
        prompt_parts.append("3. 如果违反规范，详细说明违反内容和改进建议")
        prompt_parts.append("4. 如果没有违反规范，请返回'不涉及'")
        
        # 8. 输出格式要求 - 简化版，只关注规范违反详情
  ...
  "规范检查结果": {
    ...
    "compliance_score": 85.5
        ...
        prompt_parts.append("\n重要提示：")
        prompt_parts.append("1. 如果SQL没有违反任何规范，请设置has_violations为false，violations为空数组，summary为'不涉及'")
        prompt_parts.append("2. 如果SQL违反规范，请设置has_violations为true，并在violations中详细说明")
        prompt_parts.append("3. compliance_score是合规评分（0-100%），表示SQL符合规范的程度")
        prompt_parts.append("4. 请只关注规范违反详情，不需要其他分析内容")
        ...
        self.logger.info(
            "构建增强SQL规范版prompt完成：长度=%s, SQL类型=%s, 预览前500字符=\n%s",
            len(prompt),
            sql_type.value,
            prompt_preview_head
        )
        ...
        self.logger.debug(f"构建增强SQL规范请求负载: SQL长度={len(sql_statement)}, SQL类型={sql_type.value}, 表数量={len(tables)}, prompt长度={len(prompt)}")
        return payload
    ...
    def _extract_all_suggestions(self, response_data: Dict[str, Any], answer_data: Optional[Dict[str, Any]] = None) -> List[str]:
        """
        从多个可能的位置提取建议
    ...
    def _extract_all_fields(self, response_data: Dict[str, Any], answer_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        提取其他字段
