...
# 模拟model_client_fixed.py中的_clean_extracted_sql函数
def simulate_clean_extracted_sql(sql_text):
    """模拟清理提取的SQL文本"""
    if not sql_text:
        return ""
    ...
    cleaned = simulate_fix_insert_structure_if_needed(cleaned)
...
def simulate_fix_insert_structure_if_needed(sql_text):
    """
...
result = simulate_clean_extracted_sql(problematic_sql)
...
# 分析修复函数的工作原理
print("修复函数的工作原理:")
print("1. 检查是否为INSERT语句")
print("2. 检查括号数量")
print("3. 如果没有括号，尝试找到表名和参数分界点")
print("4. 根据参数分界点分离列名和值部分")
print("5. 添加括号和VALUES关键字")

print("\n问题诊断:")
if '#{' in problematic_sql:
    print("✅ SQL中包含参数占位符 #{...}")
    param_pos = problematic_sql.find('#{')
    print(f"   参数开始位置: {param_pos}")
    before_param = problematic_sql[:param_pos]
    after_param = problematic_sql[param_pos:]
    print(f"   参数前内容长度: {len(before_param)}")
    print(f"   参数后内容长度: {len(after_param)}")
else:
    print("❌ SQL中不包含参数占位符")

# 检查表名识别
table_match = re.search(r'insert\s+into\s+([^\s(]+)', problematic_sql.lower())
if table_match:
    table_name = table_match.group(1)
    table_end = problematic_sql.lower().find(table_name) + len(table_name)
    print(f"✅ 找到表名: {table_name}, 表名结束位置: {table_end}")
else:
    print("❌ 未找到表名")