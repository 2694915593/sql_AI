...
class SQLAnalyzer:
    ...
    def analyze_single_sql(self, sql_id: int, sql_text: Optional[str] = None) -> Dict[str, Any]:
        """
        分析单个SQL语句
        
        Args:
            sql_id: SQL记录ID
            sql_text: SQL语句文本（可选，如果提供则使用此文本而不是从数据库读取）
        ...
        try:
            ...
            # 如果提供了sql_text，使用它覆盖数据库中的SQL文本
            if sql_text is not None:
                self.logger.info(f"使用提供的SQL文本（长度: {len(sql_text)}）替换数据库中的SQL文本")
                original_sql_text = sql_info['sql_text']
                sql_info['sql_text'] = sql_text
                # 记录原始SQL文本用于调试
                self.logger.debug(f"原始SQL文本: '{original_sql_text[:100]}...'")
                self.logger.debug(f"新SQL文本: '{sql_text[:100]}...'")
    ...
    def analyze_batch(self, batch_size: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for sql_info in pending_sqls:
                sql_id = sql_info['id']
                try:
                    result = self.analyze_single_sql(sql_id, None)
        ...
        try:
            ...
            for group_key, group_data in grouped_results.items():
                ...
                for sql_data in sqls:
                    ...
                    try:
                        result = self.analyze_single_sql(sql_id, sql_text)
                        if result.get('success'):
                            success_count += 1
                        else:
                            fail_count += 1
    ...
    try:
        if args.mode == 'single':
            if not args.sql_id:
                ...
                sys.exit(1)
            result = analyzer.analyze_single_sql(args.sql_id, None)
            print(f"分析结果: {result}")
