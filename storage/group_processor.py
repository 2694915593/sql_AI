...
class GroupProcessor(LogMixin):
        ...
        try:
            ...
            for file_info in files:
                ...
                if remote_enabled and system and file_path:
                    self.update_sql_from_remote_file(system, file_path, file_name, branch_name, project_id)
    ...
    def update_sql_from_remote_file(self, system: str, classpath: str, file_name: str, branch_name: str, project_id: str) -> bool:
        ...
        """
        try:
            self.logger.info(f"开始从远程服务器读取shell文件: system={system}, classpath={classpath}")
            ...
            # 3. 获取该文件的待分析SQL记录
            sql_records = self.get_pending_sqls_by_file_and_branch(file_name, branch_name, project_id)
            if not sql_records:
                self.logger.warning(f"没有找到待分析的SQL记录: {file_name}")
                return False
            ...
            for i, sql_record in enumerate(sql_records):
                if i < len(sql_statements):
                    ...
                    new_sql_text = sql_statements[i]['sql']
                    # 更新SQL文本
                    success = self.sql_extractor.update_sql_text(sql_id, new_sql_text)
                    if success:
                        updated_count += 1
                    else:
                        self.logger.warning(f"更新SQL ID {sql_id} 失败")
                else:
                    self.logger.warning(f"SQL记录数量多于提取的SQL语句数量，剩余记录将保留原内容")
                    break
                    
            self.logger.info(f"成功更新 {updated_count}/{len(sql_records)} 条SQL记录")
            return updated_count > 0
            
        except Exception as e:
            self.logger.error(f"更新远程文件SQL时发生错误: {str(e)}", exc_info=True)
            return False
